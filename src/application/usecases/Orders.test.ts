...
describe('OrdersUseCases', () => {
  ...
  it('deve criar uma nova ordem corretamente', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'RECEIVE', 'APPROVED', '', '', '', new Date(), new Date());
    ordersGatewayMock.persist.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);

    const resultado = await OrdersUseCases.setOrders('65a6f49f11aa6ff459b050e9', 1, 10, 'RECEIVE', 'APPROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
  ...
  it('deve gerar erro ao enviar customerid com menos digitos em uma nova ordem', async () => {
    ...
    try {
      await OrdersUseCases.setOrders('65a6f49f11aa6ff459b050e', 1, 10, 'RECEIVE', 'APPROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
  ...
  it('deve gerar erro ao enviar customerid errado em uma nova ordem', async () => {
    ...
    try {
      await OrdersUseCases.setOrders('1a2d1r2e1r2e1r2e1r2e1re2', 1, 10, 'RECEIVE', 'APPROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
      expect(error.message).toEqual('customerId inválid');
    }
  });
  ...
  it('deve gerar erro ao enviar status de pagamento invalido em uma nova ordem', async () => {
    ...
    try {
      await OrdersUseCases.setOrders('65a6f49f11aa6ff459b050e9', 1, 10, 'RECEIVE', 'APROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
  ...
  it('deve gerar erro ao enviar status de pagamento e status invalido em uma nova ordem', async () => {
    ...
    try {
      await OrdersUseCases.setOrders('65a6f49f11aa6ff459b050e9', 1, 10, 'CANCELED', 'APPROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
  ...
  it('deve gerar erro ao criar uma nova ordem', async () => {
    ...
    try {
      await OrdersUseCases.setOrders('65a6f49f11aa6ff459b050e9', 1, 10, 'RECEIVE', 'APPROVED', 'TESTE', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
  ...
  it('deve atualizar uma ordem por id', async () => {
    ...
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'RECEIVE', 'APPROVED', '', '', '', _rabbitMqService, ordersGatewayMock);

    expect(resultado).toEqual(expectedOrder);
  });

  it('deve atualizar uma ordem por id em progresso', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'IN_PROGRESS', 'APPROVED', '', '', '', new Date(), new Date());
    ordersGatewayMock.update.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'IN_PROGRESS', 'APPROVED', '', '', '', _rabbitMqService, ordersGatewayMock);

    expect(resultado).toEqual(expectedOrder);
  });

  it('deve atualizar uma ordem por id em finish', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'FINISH', 'APPROVED', '', '', '', new Date(), new Date());
    ordersGatewayMock.update.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'FINISH', 'APPROVED', '', '', '', _rabbitMqService, ordersGatewayMock);

    expect(resultado).toEqual(expectedOrder);
  });

  it('deve atualizar uma ordem por id em done', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'DONE', 'APPROVED', '', '', '', new Date(), new Date());
    ordersGatewayMock.update.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'DONE', 'APPROVED', '', '', '', _rabbitMqService, ordersGatewayMock);

    expect(resultado).toEqual(expectedOrder);
  });

  it('deve atualizar uma ordem por id quando denied', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'FAIL', 'DENIED', '', '', '', new Date(), new Date());
    ordersGatewayMock.update.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'FAIL', 'DENIED', '', '', '', _rabbitMqService, ordersGatewayMock);

    expect(resultado).toEqual(expectedOrder);
  });

  it('deve atualizar uma ordem por id quando canceled', async () => {
    const expectedOrder = new Orders('65a6f49f11aa6ff459b050e9', 1, '65a6f49f11aa6ff459b050e9', 1, 10, 'CANCELED', 'DENIED', '', '', '', new Date(), new Date());
    ordersGatewayMock.update.mockResolvedValue(expectedOrder);
    ordersGatewayMock.isValidId.mockResolvedValue(true);
    const resultado = await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'CANCELED', 'DENIED', '', '', '', _rabbitMqService, ordersGatewayMock);
  ...
  it('deve gerar falha ao atualizar uma ordem por id', async () => {
    ...
    try {
      await OrdersUseCases.updateOrders('65a6f49f11aa6ff459b050e9', '65a6f49f11aa6ff459b050e9', '1', 1, 10, 'RECEIVE', 'APPROVED', 'TESTE', '', '', _rabbitMqService, ordersGatewayMock);
    } catch (error: any) {
